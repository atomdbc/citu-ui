...
import { submitUserWaitlist } from '@/services/api';
...
const translations = {
  'en-US': {
    ...
    mobile: 'Mobile',
    email: 'Email Address',
    satisfaction: `How easy is it to find a property to Rent or Buy in Côte d'Ivoire?`,
    submit: 'SUBMIT',
  ...
  'fr': {
    title: 'Inscrivez-vous pour un Accès Anticipé',
    fullName: 'Nom complet',
    mobile: 'Mobile',
    email: 'Adresse e-mail',
    satisfaction: `Est-il facile de trouver une propriété à Louer ou à Acheter en Côte d'Ivoire ?`,
    submit: 'SOUMETTRE',
...
const UserEarlyAccessModal = ({ isOpen, onClose, lang = 'en-US' }) => {
  ...
  const inputClassName = "w-full p-3 sm:p-4 border border-gray-300 rounded-lg text-sm sm:text-base text-gray-900 placeholder:text-gray-500 focus:outline-none focus:ring-2 focus:ring-[#DD4440] focus:border-transparent transition-all duration-200";
      ...
      >
        ...
        >
          ...
          <div className="text-center mb-4 sm:mb-6">
            <Image src="/citu_logo.png" alt="Citu Logo" width={100} height={40} className="mx-auto sm:w-[120px] sm:h-[48px]" />
          </div>

          <h2 className="text-xl sm:text-2xl font-bold text-[#DD4440] mb-6 sm:mb-8 text-center">
            {t.title}
          </h2>
          ...
          <form onSubmit={handleSubmit} className="space-y-4 sm:space-y-6">
            <div>
              <label className="block text-gray-700 text-xs sm:text-sm font-medium mb-1">
                {t.fullName} <span className="text-[#DD4440]">{t.required}</span>
              </label>
              <input
                type="text"
                name="fullName"
            ...
            <div>
              <label className="block text-gray-700 text-xs sm:text-sm font-medium mb-1">
                {t.mobile} <span className="text-[#DD4440]">{t.required}</span>
              </label>
              <div className="flex gap-2 sm:gap-3">
                <div className="relative" ref={dropdownRef}>
                  <button
                    ...
                    className="flex items-center gap-1 sm:gap-2 bg-gray-100 p-3 sm:p-4 rounded-lg text-sm sm:text-base text-gray-700 w-[100px] sm:w-[120px]"
                  >
                  ...
                  >
                    <div className="relative">
                      <input
                        ...
                        className="sr-only"
                      />
                      <div className={`w-8 h-8 sm:w-12 sm:h-12 rounded-full border-2 transition-all duration-200
                    ...
                    <span className="text-[8px] sm:text-xs font-medium text-gray-600 text-center whitespace-pre-line uppercase tracking-wide max-w-[60px] sm:max-w-[80px] leading-tight">
                      {option.label}
                    </span>
                  </label>
                ))}
...
export default UserEarlyAccessModal;